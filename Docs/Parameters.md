...
| **SframeMode**                   | --sframe-mode          | [1-3]          | 2           | S-Frame insertion mode [1: the considered frame will be made into an S-Frame only if it is an altref frame, 2: the next altref frame will be made into an S-Frame， 3: adjuist minigop size to make an S-Frame at specific position]  |
| **SframePositions**              | --sframe-posi          | any string     | None        | S-Frame insertion positions, a list separated by ',', S-Frame process inserts by the specified frame numbers (0 based), only applicable for mode 3  |
